...
export default async function RequestPage({ searchParams }: { searchParams: Promise<{ authority?: string; need?: string }> }) {
  const { authority, need } = await searchParams;
  return (
    <PageShell>
      ...
      <RequestWorkflow initialAuthority={authority} initialRequest={need} />
    </PageShell>
  );
}
